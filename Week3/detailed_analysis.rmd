...
ggplot(mapping = aes(x = time, y = mRNA)) +
        ...
        scale_colour_manual(name = "", values = group.cols,
                            limits = c("ref", "5 less", "2 less", "2 more", "5 more"))
        guides(color = guide_legend(title = "",
...
ggplot(data = MPL_data, mapping = aes(x = time, y = mRNA)) +
        geom_line(data = model_k.re_ref, linetype = "dashed", aes(color = "ref")) +
...
To simulate this situation, $$k_{s_R}$$ needs to be put to zero.
...
params <- c(
        ...
        k.d_R = 0.0572, # 1/h, 1e orde k voor afbraak van de receptor
        k.d_Rm = 0.612, # 1e orde k voor GR mRNA afbraak
        k.s_R = 0, # 1e orde k voor aanmaak receptor
        D = (20 * 1000)/374.471 # nmol/L, als molgewicht[MPL] = 374.471 g/mol
)
...
ggplot(data = no.Receptor, mapping = aes(x = time, y = mRNA)) +
        geom_line(aes(color = "No receptor")) +
        geom_line(data = model_20, aes(color = "Model 20")) +
        labs(x = "mRNA (fmol/g)", y = "Time", title = "No receptor synthesis (mRNA)") +
        scale_colour_manual(name = "", values = group.cols[1:2],
                            limits = c("No receptor", "Model 20"))

ggplot(data = no.Receptor, mapping = aes(x = time, y = Free_receptor)) +
        geom_line(aes(color = "No receptor")) +
        geom_line(data = model_20, aes(color = "Model 20")) +
...
ggplot(data = no.Receptor, mapping = aes(x = time, y = MPL_conc)) +
        geom_line(aes(color = "No receptor")) +
        geom_line(data = model_20, aes(color = "Model 20")) +
