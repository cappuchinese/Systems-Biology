...
A model can be proposed that represents the growth dynamics of fruit trees, where the supply of irrigation is evidenced in the form of a pulse, and thus be described by impulsive differential equations \ref{eq:eq1}.
...
After several tests were run to ensure parity with the described model in the paper. See Appendix B. File: [results1](results1.R).
...
See Appendix C. File: [results2](results2.R).

Another few models were made to compare different irrigation intervals, as well as a model for watering the soil as soon as almost all water was drained from it. See Appendix D and Appendix E. Files: [results3](results3.R) \& [results4](results4.R).
...
```{r res1, file = 'results1.R', fig.cap = "\\label{fig:fig2}The different simulations", fig.height = 7}
```
