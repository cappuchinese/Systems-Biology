...
## Expansion
The following simulations show the difference between the expanded model accounting for day and night cycles in red, and the original model from \ref{ref:ref1} in black.
...
```{r res3, file = 'results3.R', fig.cap = "\\label{fig:fig4}The simulation comparisons", fig.height = 7}
```
...
```{r res4, file = 'results4.R', fig.cap = "\\label{fig:fig5}The edge case simulation", fig.height = 7}
```
...
[//]: # (Page : Appendix)
# Appendix
```{r appendix, child = 'appendix.rmd', attr.source='.numberLines', echo = TRUE, fig.show='hide'}
```
